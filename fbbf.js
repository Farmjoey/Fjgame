...
let birdY = 400;
let birdVelocity = 0;
let gravity = 0.10; // lowered from 0.13 for easier gameplay
const jump = -2.0;
const mobileJump = -2.0;
const pipeWidth = 70;
let pipeGap = 150;
let pipeSpeed = 4.0; // doubled from 2.0
let pipes = [];
let score = 0;
let highScore = 0;
...
let godMode = false;
let backgroundX = 0;
let backgroundSpeed = 2.4; // doubled from 1.2
let coins = 0;
...
let highScoreAddedThisGame = false; // Add this at the top with other game variables
    ...
    if (coins >= cost) {
        ...
        switch(type) {
            ...
            case 'slowerPipes':
                ...
                break;
            case 'lowerGravity':
                gravity = 0.09; // less powerful than before, but still lower than default
                break;
            case 'tripleCoins':
...
function drawBackground(deltaTime = 1) {
...
function gameLoop() {
  // Calculate deltaTime (normalized to 60 FPS)
  let now = performance.now();
  let deltaTime = (now - lastFrameTime) / 16.67; // 16.67ms = 60 FPS
  lastFrameTime = now;
  ...
  if (gameOver) {
    ...
    if (!highScoreAddedThisGame && score > 0) {
      highScore += score;
      saveHighScore();
      highScoreAddedThisGame = true;
    }
